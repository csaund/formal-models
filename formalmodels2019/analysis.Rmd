...
```{r}
get_n_percent_of_data <- function(data, n) {
  num_samples <- nrow(data) * ( n / 100)
  print(num_samples)
  # get n percent of data
  return(sample_n(data, num_samples))
}
...
split_data_n_percent <- function(data, n) {
  ...
  sample_size <- floor(0.90 * nrow(movie_data))
}


```

### test on data that isn't in set
```{r}
